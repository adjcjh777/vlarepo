...
def handle_hook(payload: Dict[str, Any], store: Optional[AgentStore] = None) -> Dict[str, Any]:
    ...
    if session_id:
        ...
        if "stop" in event.lower():
            ...
            claimed_count = len(store.claim_pending_messages(agent))
    context = (
    ...
    if claimed_count:
        context += (
            " Agent Bus claimed %d queued message(s) for this session; run get_inbox "
            "or agent-bus inbox --target <this session/name> before starting new work."
        ) % claimed_count
    return {
