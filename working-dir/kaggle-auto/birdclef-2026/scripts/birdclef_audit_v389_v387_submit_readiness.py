#!/usr/bin/env python3
"""Submit-readiness audit for v387.

This script never submits. It verifies whether v387 has enough evidence before
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--output-csv", default=str(EXPERIMENTS / "v389_v387_submit_readiness_20260524.csv"))
    ap.add_argument("--report", default=str(EXPERIMENTS / "v389_v387_submit_readiness_20260524.md"))
    ap.add_argument("--runtime-json", default=str(ARTIFACTS / "runtime_v389_v387_submit_readiness_20260524.json"))
...
def live_submissions() -> dict[str, object]:
    ...
    for sub in subs[:20]:
        ...
        if "v387" in desc.lower() or "v386 static" in desc.lower():
            duplicate = True
...
def main() -> int:
    ...
    add(
        rows,
        "v387_runmode_ready",
        run_decision in {"READY-v387-runmode-schema-runtime-proof", "READY-v387-runmode-dryrun-runtime-proof"},
        f"decision={run_decision} runtime_seconds={runmode.get('runtime_seconds')}",
    )
    add(rows, "v387_runmode_not_failed", not run_decision.startswith("REJECT"), f"decision={run_decision}")
    add(
        rows,
        "v387_static_ready",
        static.get("decision") == "READY-v387-v386-static-materializer-audit-NO-PUSH-NO-SUBMIT",
        f"decision={static.get('decision')}",
    )
    add(rows, "v387_static_no_blockers", audit.get("block_count") == 0, f"block_count={audit.get('block_count')}")
    ...
    add(rows, "no_duplicate_v387_submission", live.get("duplicate_v387") is False, f"duplicate_v387={live.get('duplicate_v387')}")
    add(rows, "explicit_competition_submit_not_made", True, "this audit has no submit call")

    blockers = [r for r in rows if r.blocker]
    if blockers:
        decision = "WAIT-v387-submit-readiness-blocked-NO-SUBMIT"
        next_action = "Resolve blockers before any separate v387 final-submit gate."
    else:
        decision = "READY-v387-submit-readiness-AWAIT-EXPLICIT-SUBMIT-GATE"
    ...
    runtime = {
        ...
        "experiment_id": "v389-v387-submit-readiness",
        "decision": decision,
    ...
    lines = [
        ...
        f"- Duplicate v387: `{live.get('duplicate_v387')}`",
        "",
        "## Checks",
        "",
