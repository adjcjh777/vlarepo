#!/usr/bin/env python3
"""Prepare v103 original guarded macro-risk rescue candidate from private v86."""
...
DEST_DIR = Path("birdclef-2026/notebooks/v103-guarded-macro-risk-rescue")
SOURCE_NOTEBOOK = "submission.ipynb"
SOURCE_METADATA = "kernel-metadata.json"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v103-guarded-macro-rescue"
NEW_TITLE = "bc26-v103-guarded-macro-rescue"
...
NEW_BLOCK = """# V103 original guarded macro-risk rescue policy.
...
print(f"V103 guarded macro-risk rescue counts: {v103_counts}")
print(f"V103 class rescue mix range/mean: [{rescue_mix.min():.3f}, {rescue_mix.max():.3f}] / {rescue_mix.mean():.4f}")
print(f"V103 effective rescue mix range/mean: [{effective_rescue_mix.min():.3f}, {effective_rescue_mix.max():.3f}] / {effective_rescue_mix.mean():.4f}")
print(f"V103 components: macro mean/std={macro_probs.mean():.6f}/{macro_probs.std():.6f}; v84_like mean/std={v84_like.mean():.6f}/{v84_like.std():.6f}; rescue_view mean/std={rescue_view.mean():.6f}/{rescue_view.std():.6f}; dualprobe mean/std={probe_consensus_probs.mean():.6f}/{probe_consensus_probs.std():.6f}")
print(f"OOF refs: Perch raw={perch_auc_raw_ref.mean():.3f}, Perch cal={perch_auc_cal_ref.mean():.3f}, SED={sed_auc_ref.mean():.3f}, delta={auc_delta.mean():+.3f}")
print(f"SED weights sedfirst={sed_w_sedfirst.mean():.3f}, adaptive={sed_w_adaptive.mean():.3f}, safety={sed_w_safety.mean():.3f}")
print(f"View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards")
print(f"Prior strength mean={prior_strength.mean():.3f}, applied mean={prior_mix.mean():.4f}, file scale mean={file_scale.mean():.3f}")
print(f"Spike-smoothed score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"Score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")
"""
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V103 Original: Guarded Macro-Risk Rescue\\n",
        "\\n",
        "Provenance: this notebook continues the private original v13/v20/v23/v70/v73/v76/v83/v84/v86/v102 fast-ONNX line in this workspace. It uses public Perch v2 ONNX and public distilled SED ONNX model assets as external inputs with attribution, but the guarded macro-risk rescue policy is written here. Public notebooks remain idea references only; no public notebook code is copied.\\n",
        "\\n",
        "Experiment: v103 conservatizes v102 by reducing rescue strength, requiring positive per-cell rescue evidence, and adding a row-level top-hit preservation guard. The goal is to keep original macro-risk innovation while reducing micro/top-hit damage. Run-mode only until validated.\\n",
    ]
    ...
    (dest_dir / "ATTRIBUTION.md").write_text(
        "\n".join(
            [
                "# Attribution",
                "",
                "This candidate continues the private original v86/v102 line.",
                "",
                "Original v103 change:",
                "- keep v102's class-wise macro-risk rescue policy;",
                "- reduce rescue strength for weak mid-support classes;",
                "- require positive per-cell rescue evidence before full rescue mixing;",
                "- add a row-level top-hit preservation guard for confident anchor winners;",
                ...
                "or without v103 Run-mode evidence and candidate-specific compliance review.",
                "",
            ]
        )
    )
