#!/usr/bin/env python3
"""Prepare v104 Perch-only guarded macro rescue from v103.

v104 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It keeps the v103 original guard idea but rebuilds train Perch features inside
the notebook and replaces the SED branch with a Perch-only surrogate view.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v104-perch-only-guarded-rescue")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v104-perch-only-guarded"
NEW_TITLE = "bc26-v104-perch-only-guarded"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V104 Original: Perch-Only Guarded Macro Rescue\n",
        "\n",
        "Provenance: this notebook continues the private original v86/v102/v103 fast-ONNX line. "
        "Unlike v103, it removes the unknown-license runtime dependencies `jaejohn/perch-meta` "
        ...
        "\n",
        "Experiment: v104 preserves v103's positive-rescue and top-hit guard idea while reducing "
        "prize-route compliance risk. Run-mode only until schema, runtime, proxy, and compliance "
        "evidence pass.\n",
    ]
    ...
    cache_cell = (
        ...
        + 'CACHE_DIR = None\nprint("External Perch cache disabled for v104 license-reduced run")\n'
    )
    ...
    cell7 += 'print("V104: Perch train cache will be rebuilt in-notebook after inference helpers are defined")\n'
    ...
    build_train_src = """# V104: rebuild train Perch features in-notebook to avoid the unknown-license perch-meta cache.
train_paths = [BASE / "train_soundscapes" / fn for fn in full_files]
train_paths = [p for p in train_paths if p.exists()]
print(f"V104 rebuilding Perch train features for {len(train_paths)} files")
meta_full, scores_full_raw, emb_full, _spatial_train_unused = run_perch_fast(
    train_paths, capture_spatial=False, desc="V104 Perch train feature rebuild"
)
...
print(f"V104 rebuilt train features: meta={meta_full.shape}, scores={scores_full_raw.shape}, emb={emb_full.shape}")
"""

    # Replace SED branch with a no-external-license Perch surrogate.
    sed_surrogate_src = """# V104 license-reduced branch: no external SED folds.
...
print(f"V104 Perch-only surrogate view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
"""

    final_cell = "".join(cells[15]["source"])
    final_cell = final_cell.replace(
        "# V103 original guarded macro-risk rescue policy.",
        "# V104 original Perch-only guarded macro-risk rescue policy.",
    ).replace(
        ...
        "V104 Perch-only guarded macro-risk rescue counts",
    ).replace(
        "V103 class rescue mix range/mean",
        "V104 class rescue mix range/mean",
    ).replace(
        "V103 effective rescue mix range/mean",
        "V104 effective rescue mix range/mean",
    ).replace(
        "V103 components:",
        "V104 components:",
    ).replace(
        "View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards",
        "View mix: v104 = Perch-only original guarded macro-risk rescue without unknown-license cache/SED dependencies",
    )
    ...
    (dest_dir / "ATTRIBUTION.md").write_text(
        "\n".join(
            [
                "# Attribution",
                "",
                "This candidate continues the private original v86/v102/v103 line.",
                "",
                "Original v104 change:",
                ...
                "- keeps the v103 positive-rescue and top-hit guard idea with more conservative rescue strengths;",
                "- keeps CPU-only/no-internet metadata and public, recorded Perch inputs.",
                "",
                "Run-mode only until v104 runtime, schema, proxy, and compliance evidence pass.",
                "",
            ]
        )
    )
