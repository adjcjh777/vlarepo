#!/usr/bin/env python3
"""Prepare v106 rank-dominant Perch-only guarded macro rescue from v103.

v105 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It keeps the v104/v105 license-reduced dependency set and tests a much more
aggressive row-rank restoration ceiling for the Perch-only path.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v106-rankdominant-perch-guarded")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v106-rankdominant-perch"
NEW_TITLE = "bc26-v106-rankdominant-perch"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V106 Original: Rank-Dominant Perch Guarded Rescue\n",
        "\n",
        ...
        "\n",
        "Experiment: v106 preserves v105's license-clean dependency set but makes the "
        "row-rank restoration much stronger to test whether Perch/probe views can recover "
        "top-hit/top5 without external SED. Run-mode only; do not submit unless proxy and "
    ...
    cache_cell = (
        ...
        + 'CACHE_DIR = None\nprint("External Perch cache disabled for v106 license-reduced run")\n'
    )
    ...
    cell7 += 'print("V106: Perch train cache will be rebuilt in-notebook after inference helpers are defined")\n'
    ...
    build_train_src = """# V106: rebuild train Perch features in-notebook to avoid the unknown-license perch-meta cache.
train_paths = [BASE / "train_soundscapes" / fn for fn in full_files]
train_paths = [p for p in train_paths if p.exists()]
print(f"V106 rebuilding Perch train features for {len(train_paths)} files")
meta_full, scores_full_raw, emb_full, _spatial_train_unused = run_perch_fast(
    train_paths, capture_spatial=False, desc="V106 Perch train feature rebuild"
)
...
print(f"V106 rebuilt train features: meta={meta_full.shape}, scores={scores_full_raw.shape}, emb={emb_full.shape}")
"""

    # Replace SED branch with a no-external-license Perch surrogate.
    sed_surrogate_src = """# V106 license-reduced branch: no external SED folds.
...
print(f"V106 Perch-only surrogate view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
"""

    final_cell = "".join(cells[15]["source"])
    final_cell = final_cell.replace(
        "# V103 original guarded macro-risk rescue policy.",
        "# V106 original rank-dominant Perch-only guarded macro-risk rescue policy.",
    ).replace(
        ...
        "V106 rank-dominant Perch-only guarded macro-risk rescue counts",
    ).replace(
        "V103 class rescue mix range/mean",
        "V106 class rescue mix range/mean",
    ).replace(
        "V103 effective rescue mix range/mean",
        "V106 effective rescue mix range/mean",
    ).replace(
        "V103 components:",
        "V106 components:",
    ).replace(
        "View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards",
        "View mix: v106 = Perch-only guarded rescue plus rank-dominant restoration without unknown-license cache/SED dependencies",
    )
    restore_block = """
# V106 rank-dominant restoration. v105 was still too timid: top-hit/top5 did
# not move. This layer intentionally tests the ceiling of Perch/probe row
# ranking while keeping rare/high-AUC guards conservative.
rank_restore_view = _rank_power(
    0.46 * view_sedfirst + 0.24 * view_perch + 0.20 * probe_consensus_probs + 0.10 * view_adaptive,
    0.70,
).astype(np.float32)
restore_mask = (~rare_guard) & (~protect_guard) & (rank_restore_view > 0.52)
restore_mix = np.where(restore_mask, 0.62, 0.0).astype(np.float32)
final_probs = ((1.0 - restore_mix) * final_probs + restore_mix * rank_restore_view).astype(np.float32)

# Preserve confident row winners from the restored rank view. This is aggressive
# by design and will be rejected if proxy macro/top-hit tradeoff is poor.
restored_top = rank_restore_view.max(axis=1, keepdims=True)
restored_top_guard = (rank_restore_view >= (restored_top - 0.020)) & (restored_top >= 0.78) & (~rare_guard)
final_probs = np.where(restored_top_guard, 0.50 * final_probs + 0.50 * rank_restore_view, final_probs).astype(np.float32)
print(f"V106 rank-dominant restore cells: restore={int(restore_mask.sum())}, top_guard={int(restored_top_guard.sum())}")
print(f"V106 post-restore score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"V106 post-restore score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")
"""
    final_cell = final_cell.replace(
    ...
    (dest_dir / "ATTRIBUTION.md").write_text(
        "\n".join(
            [
                "# Attribution",
                "",
                "This candidate continues the private original v86/v102/v103/v104/v105 line.",
                "",
                "Original v106 change:",
                ...
                "- keeps the v103/v104/v105 positive-rescue and top-hit guard idea with conservative class rescue strengths;",
                "- adds an aggressive rank-dominant restoration layer from license-clean Perch/probe views;",
                "- keeps CPU-only/no-internet metadata and public, recorded Perch inputs.",
                "",
                "Run-mode only until v106 runtime, schema, proxy, and compliance evidence pass.",
                "",
            ]
        )
    )
