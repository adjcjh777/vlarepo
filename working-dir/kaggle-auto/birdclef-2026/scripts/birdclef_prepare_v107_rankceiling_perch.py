#!/usr/bin/env python3
"""Prepare v107 rank-ceiling Perch-only guarded macro rescue from v103.

v105 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It keeps the v104-v106 license-reduced dependency set and tests the near-pure
rank-view ceiling for the Perch-only path.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v107-rankceiling-perch-guarded")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v107-rankceiling-perch"
NEW_TITLE = "bc26-v107-rankceiling-perch"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V107 Original: Rank-Ceiling Perch Guarded Rescue\n",
        "\n",
        ...
        "\n",
        "Experiment: v107 preserves v106's license-clean dependency set but makes the "
        "row-rank restoration near-dominant to test the ceiling of Perch/probe views without "
        "external SED. Run-mode only; do not submit unless proxy and "
    ...
    cache_cell = (
        ...
        + 'CACHE_DIR = None\nprint("External Perch cache disabled for v107 license-reduced run")\n'
    )
    ...
    cell7 += 'print("V107: Perch train cache will be rebuilt in-notebook after inference helpers are defined")\n'
    ...
    build_train_src = """# V107: rebuild train Perch features in-notebook to avoid the unknown-license perch-meta cache.
train_paths = [BASE / "train_soundscapes" / fn for fn in full_files]
train_paths = [p for p in train_paths if p.exists()]
print(f"V107 rebuilding Perch train features for {len(train_paths)} files")
meta_full, scores_full_raw, emb_full, _spatial_train_unused = run_perch_fast(
    train_paths, capture_spatial=False, desc="V107 Perch train feature rebuild"
)
...
print(f"V107 rebuilt train features: meta={meta_full.shape}, scores={scores_full_raw.shape}, emb={emb_full.shape}")
"""

    # Replace SED branch with a no-external-license Perch surrogate.
    sed_surrogate_src = """# V107 license-reduced branch: no external SED folds.
...
print(f"V107 Perch-only surrogate view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
"""

    final_cell = "".join(cells[15]["source"])
    final_cell = final_cell.replace(
        "# V103 original guarded macro-risk rescue policy.",
        "# V107 original rank-ceiling Perch-only guarded macro-risk rescue policy.",
    ).replace(
        ...
        "V107 rank-ceiling Perch-only guarded macro-risk rescue counts",
    ).replace(
        "V103 class rescue mix range/mean",
        "V107 class rescue mix range/mean",
    ).replace(
        "V103 effective rescue mix range/mean",
        "V107 effective rescue mix range/mean",
    ).replace(
        "V103 components:",
        "V107 components:",
    ).replace(
        "View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards",
        "View mix: v107 = Perch-only guarded rescue plus near-pure rank-ceiling restoration without unknown-license cache/SED dependencies",
    )
    restore_block = """
# V107 rank-ceiling restoration. v106 improved top5 but remained below the
# v103 target. This layer intentionally pushes near-pure row ranking for
# non-rare, non-anchor cells to test the Perch/probe information ceiling.
rank_restore_view = _rank_power(
    0.50 * view_sedfirst + 0.24 * view_perch + 0.18 * probe_consensus_probs + 0.08 * view_adaptive,
    0.62,
).astype(np.float32)
restore_mask = (~rare_guard) & (~protect_guard)
restore_mix = np.where(restore_mask, 0.88, 0.0).astype(np.float32)
final_probs = ((1.0 - restore_mix) * final_probs + restore_mix * rank_restore_view).astype(np.float32)

# Preserve confident row winners from the restored rank view. This is an upper
# bound probe, not a default submission policy.
restored_top = rank_restore_view.max(axis=1, keepdims=True)
restored_top_guard = (rank_restore_view >= (restored_top - 0.030)) & (restored_top >= 0.72) & (~rare_guard)
final_probs = np.where(restored_top_guard, 0.20 * final_probs + 0.80 * rank_restore_view, final_probs).astype(np.float32)
print(f"V107 rank-ceiling restore cells: restore={int(restore_mask.sum())}, top_guard={int(restored_top_guard.sum())}")
print(f"V107 post-restore score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"V107 post-restore score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")
"""
    final_cell = final_cell.replace(
    ...
    (dest_dir / "ATTRIBUTION.md").write_text(
        "\n".join(
            [
                "# Attribution",
                "",
                "This candidate continues the private original v86/v102/v103/v104/v105/v106 line.",
                "",
                "Original v107 change:",
                ...
                "- adds a near-pure rank-ceiling restoration layer from license-clean Perch/probe views;",
                "- keeps CPU-only/no-internet metadata and public, recorded Perch inputs.",
                "",
                "Run-mode only until v107 runtime, schema, proxy, and compliance evidence pass.",
                "",
            ]
        )
    )
