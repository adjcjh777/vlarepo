#!/usr/bin/env python3
"""Prepare v108 EcoProto Perch-only guarded rescue from v103.

v108 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It keeps the license-clean Perch-only dependency set, but adds an original
ecological prototype rescue: class prototypes are built from competition
train-soundscape Perch embeddings, then gated by site/hour priors and temporal
consistency. This adds our own ranking signal instead of merely tracking public
notebook behavior.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v108-ecoproto-perch-guarded")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v108-ecoproto-perch"
NEW_TITLE = "bc26-v108-ecoproto-perch"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V108 Original: EcoProto Perch Guarded Rescue\n",
        "\n",
        ...
        "\n",
        "Experiment: v108 adds an original ecological prototype rescue: train-window "
        "Perch embeddings form class prototypes, then test rows are rescued only when "
        "prototype similarity agrees with site/hour priors and temporal context. "
    ...
    cache_cell = (
        ...
        + 'CACHE_DIR = None\nprint("External Perch cache disabled for v108 EcoProto license-reduced run")\n'
    )
    ...
    cell7 += 'print("V108: Perch train cache will be rebuilt in-notebook after inference helpers are defined")\n'
    ...
    build_train_src = """# V108: rebuild train Perch features in-notebook to avoid the unknown-license perch-meta cache.
train_paths = [BASE / "train_soundscapes" / fn for fn in full_files]
train_paths = [p for p in train_paths if p.exists()]
print(f"V108 rebuilding Perch train features for {len(train_paths)} files")
meta_full, scores_full_raw, emb_full, _spatial_train_unused = run_perch_fast(
    train_paths, capture_spatial=False, desc="V108 Perch train feature rebuild"
)
...
print(f"V108 rebuilt train features: meta={meta_full.shape}, scores={scores_full_raw.shape}, emb={emb_full.shape}")
"""

    # Replace SED branch with a no-external-license Perch surrogate.
    sed_surrogate_src = """# V108 license-reduced branch: no external SED folds.
...
print(f"V108 Perch-only surrogate view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
"""

    final_cell = "".join(cells[15]["source"])
    final_cell = final_cell.replace(
        "# V103 original guarded macro-risk rescue policy.",
        "# V108 original EcoProto Perch-only guarded macro-risk rescue policy.",
    ).replace(
        ...
        "V108 EcoProto Perch-only guarded macro-risk rescue counts",
    ).replace(
        "V103 class rescue mix range/mean",
        "V108 class rescue mix range/mean",
    ).replace(
        "V103 effective rescue mix range/mean",
        "V108 effective rescue mix range/mean",
    ).replace(
        "V103 components:",
        "V108 components:",
    ).replace(
        "View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards",
        "View mix: v108 = Perch-only guarded rescue plus original EcoProto prototype/site-hour/temporal rescue without unknown-license cache/SED dependencies",
    )
    restore_block = """
# V108 EcoProto rescue. This is an original, license-clean signal built only
...
def _ecoproto_rank():
    if emb_test is None:
        print("V108 EcoProto unavailable: no embedding output")
    ...
    print(f"V108 EcoProto active classes: {int(active.sum())}/{N_CLASSES}; sim range=[{sim.min():.4f},{sim.max():.4f}]")
...
print(f"V108 EcoProto cells: mix={int((eco_mix > 0).sum())}, temporal={int(temporal_mask.sum())}, class_gate={int(class_eco_gate.sum())}")
print(f"V108 EcoProto mix mean/max: {eco_mix.mean():.5f}/{eco_mix.max():.5f}; temporal mean={temporal_mix.mean():.5f}")
print(f"V108 post-EcoProto score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"V108 post-EcoProto score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")
"""
    final_cell = final_cell.replace(
    ...
    (dest_dir / "ATTRIBUTION.md").write_text(
        "\n".join(
            [
                "# Attribution",
                "",
                "This candidate continues the private original v86/v102/v103/v104-v107 line.",
                "",
                "Original v108 change:",
                ...
                "",
                "Run-mode only until v108 runtime, schema, proxy, and compliance evidence pass.",
                "",
            ]
        )
    )
