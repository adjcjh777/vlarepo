#!/usr/bin/env python3
"""Prepare v110 EcoProto clean blend from v103.

v110 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It keeps the license-clean Perch-only dependency set and self-replays both the
v107-like rank-ceiling branch and the v109 EcoProto rank-launch branch inside
one notebook, then blends them with the local clean-branch probe ratio.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v110-ecoproto-clean-blend")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v110-ecoproto-cleanblend"
NEW_TITLE = "bc26-v110-ecoproto-cleanblend"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V110 Original: EcoProto Clean Blend\n",
        "\n",
        ...
        "\n",
        "Experiment: v110 self-replays a v107-like rank-ceiling branch and the v109 "
        "EcoProto rank-launch branch inside one clean notebook, then blends them at "
        "`0.4/0.6` according to the local clean-branch probe. "
    ...
    cache_cell = (
        ...
        + 'CACHE_DIR = None\nprint("External Perch cache disabled for v110 EcoProto clean-blend run")\n'
    )
    ...
    cell7 += 'print("V110: Perch train cache will be rebuilt in-notebook after inference helpers are defined")\n'
    ...
    build_train_src = """# V110: rebuild train Perch features in-notebook to avoid the unknown-license perch-meta cache.
train_paths = [BASE / "train_soundscapes" / fn for fn in full_files]
train_paths = [p for p in train_paths if p.exists()]
print(f"V110 rebuilding Perch train features for {len(train_paths)} files")
meta_full, scores_full_raw, emb_full, _spatial_train_unused = run_perch_fast(
    train_paths, capture_spatial=False, desc="V110 Perch train feature rebuild"
)
...
print(f"V110 rebuilt train features: meta={meta_full.shape}, scores={scores_full_raw.shape}, emb={emb_full.shape}")
"""

    # Replace SED branch with a no-external-license Perch surrogate.
    sed_surrogate_src = """# V110 license-reduced branch: no external SED folds.
...
print(f"V110 Perch-only surrogate view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
"""

    final_cell = "".join(cells[15]["source"])
    final_cell = final_cell.replace(
        "# V103 original guarded macro-risk rescue policy.",
        "# V110 original EcoProto clean-blend guarded macro-risk rescue policy.",
    ).replace(
        ...
        "V110 EcoProto clean-blend guarded macro-risk rescue counts",
    ).replace(
        "V103 class rescue mix range/mean",
        "V110 class rescue mix range/mean",
    ).replace(
        "V103 effective rescue mix range/mean",
        "V110 effective rescue mix range/mean",
    ).replace(
        "V103 components:",
        "V110 components:",
    ).replace(
        "View mix: v103 = v102 original macro-risk rescue plus positive-rescue and top-hit guards",
        "View mix: v110 = self-contained v107-like rank ceiling plus EcoProto rank-launch clean blend",
    )
    restore_block = """
clean_guarded_base = final_probs.copy()

# V110 EcoProto rescue. This is an original, license-clean signal built only
...
def _ecoproto_rank():
    if emb_test is None:
        print("V110 EcoProto unavailable: no embedding output")
    ...
    print(f"V110 EcoProto active classes: {int(active.sum())}/{N_CLASSES}; sim range=[{sim.min():.4f},{sim.max():.4f}]")
...
print(f"V110 EcoProto cells: mix={int((eco_mix > 0).sum())}, temporal={int(temporal_mask.sum())}, class_gate={int(class_eco_gate.sum())}")
print(f"V110 EcoProto mix mean/max: {eco_mix.mean():.5f}/{eco_mix.max():.5f}; temporal mean={temporal_mix.mean():.5f}")
print(f"V110 post-EcoProto score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"V110 post-EcoProto score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")

# V110 self-contained clean blend. First reconstruct a v107-like rank-ceiling
...
print(f"V110 v107-like cells: restore={int(v107_restore_mask.sum())}, top_guard={int(v107_top_guard.sum())}")
print(f"V110 EcoProto rank-launch cells: launch={int(launch_mask.sum())}, top_guard={int(rank_top_guard.sum())}")
print(f"V110 clean blend weights: v107_like=0.40, v109_like=0.60")
print(f"V110 post-cleanblend score range: [{final_probs.min():.6f}, {final_probs.max():.6f}]")
print(f"V110 post-cleanblend score mean/std: {final_probs.mean():.6f}/{final_probs.std():.6f}")
"""
    final_cell = final_cell.replace(
