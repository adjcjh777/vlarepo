#!/usr/bin/env python3
"""Prepare v112 clean SED remap EcoProto blend from v103.

v111 removes runtime dependencies with unknown Kaggle metadata licenses:
- jaejohn/perch-meta
- tuckerarrants/bc2026-distilled-sed-public

It adds an original self-calibration layer on top of the audited CC0 Kaggle SED
ONNX asset: train soundscape windows are used to infer a conservative
output-column remap and per-class trust strength before blending with EcoProto.
The goal is to test whether v111 failed because the clean SED class order or
calibration is misaligned, without returning to unknown-license public
SED/cache assets.
"""
...
DEST_DIR = Path("birdclef-2026/notebooks/v112-clean-sed-remap-ecoproto")
NOTEBOOK = "submission.ipynb"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v112-clean-sed-remap"
NEW_TITLE = "bc26-v112-clean-sed-remap"
...
def patch_notebook(path: Path) -> None:
    ...
    cells[0]["source"] = [
        "# BirdCLEF+ 2026 - V112 Original: Clean SED Remap EcoProto Blend\n",
        "\n",
        ...
        "\n",
        "Experiment: v112 keeps the EcoProto clean-blend structure but inserts an "
        "original clean-SED remap layer learned from train soundscape windows. "
        "The remap only trusts a replacement output column when train-window "
        "AUC/correlation beats the same-index column by a margin, and it emits "
        "diagnostics for every class. "
...
"""
    ...
    sed_surrogate_src = """# V112 original branch: audited CC0 raw-audio SED with train-window column remap.
...
# Stabilize the clean SED branch for AUC-style use: keep calibrated probabilities
...
assert sed_rows == meta_test["row_id"].astype(str).tolist(), "Clean SED row_id order mismatch"
...
remap_diag.to_csv("v112_clean_sed_remap_diagnostics.csv", index=False)
print(f"V112 clean SED remap view: shape={sed_probs.shape}, range=[{sed_probs.min():.6f}, {sed_probs.max():.6f}]")
print(f"V112 clean SED remaps accepted: {int((remap_cols != identity_cols).sum())}/{N_CLASSES}; strength mean/max={remap_strength.mean():.4f}/{remap_strength.max():.4f}")
print(f"V112 clean SED train AUC ref: mean={np.nanmean(sed_per_class_auc):.4f}, min={np.nanmin(sed_per_class_auc):.4f}, active={(np.isfinite(sed_per_class_auc)).sum()}")
print("V112 remap diagnostics saved: v112_clean_sed_remap_diagnostics.csv")
"""
...
"""
    ...
    for cell in nb["cells"]:
        ...
        if isinstance(src, list):
            src_text = "".join(src).replace("V111", "V112").replace("v111", "v112")
            cell["source"] = src_text.splitlines(keepends=True)
        elif isinstance(src, str):
            cell["source"] = src.replace("V111", "V112").replace("v111", "v112")
