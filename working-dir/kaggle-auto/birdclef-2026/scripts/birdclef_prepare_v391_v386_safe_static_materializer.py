#!/usr/bin/env python3
"""Materialize the v386 safe-final parameter route as a static notebook.
...
DEST = ROOT / "birdclef-2026/notebooks/v391-v386-safe-static-distill"
NEW_KERNEL_ID = "junhaochengadjcjh7u7/bc26-v391-v386-safe-static-distill"
NEW_TITLE = "bc26-v391-v386-safe-static-distill"
SPEC_PATH = ROOT / "experiments/v391_v386_safe_static_distill_spec.json"
LOCAL_SUB = ROOT / "experiments/v391_v386_safe_static_distill_local_submission.csv"
CSV = ROOT / "experiments/v391_v386_safe_static_materializer_20260524.csv"
REPORT = ROOT / "experiments/v391_v386_safe_static_materializer_20260524.md"
RUNTIME = ROOT / "artifacts/runtime_v391_v386_safe_static_materializer_20260524.json"
LINEAGE = ROOT / "artifacts/lineage_v391_v386_safe_static_materializer_20260524.json"
MARKER = "CODEX_V391_V386_SAFE_STATIC_DISTILL_PATCH"
    ...
    spec.update(
        {
            ...
            "patch_marker": MARKER,
            "source_route": "v383-param-stress-safe-final-blend1-delta0.0025-pr0.7-scale1.25",
            "blend_weight": 1.0,
            "min_delta": 0.0025,
            "min_prior_rank": 0.7,
            "delta_scale_materialized_into_beta": 1.25,
            "runtime_policy": "safe_final runtime uses v107 submission, competition train_soundscape labels, row_id context, fixed coefficients scaled by v386, stricter prior-rank/delta gates, and tolerates v107 train-row dry-run fallback",
        }
    )
...
def main() -> int:
    ...
    nb_text = nb_text.replace("v383_static_distill_summary.csv", "v391_static_distill_summary.csv")
    nb_text = nb_text.replace("v383_static_distill_diagnostics.csv", "v391_static_distill_diagnostics.csv")
    nb_text = nb_text.replace(
        "v383 dry-run tolerant static distill patch complete",
        "v391 dry-run tolerant static distill patch complete",
    )
    nb_text = nb_text.replace(
        "v383 dry-run row mismatch tolerated",
        "v391 dry-run row mismatch tolerated",
    )
    ...
    if audit["decision"] == "READY-v383-dryrun-tolerant-static-audit-NO-PUSH-NO-SUBMIT":
        audit["decision"] = "READY-v391-v386-safe-static-materializer-audit-NO-PUSH-NO-SUBMIT"
        audit["next_recommended_action"] = "Build a guarded v391 Run-mode push/audit pair only if a safe_final slot is needed after v387/v390."
    v383.write_outputs(info, spec, metrics, audit, time.time() - started)

    runtime = json.loads(RUNTIME.read_text(encoding="utf-8"))
    runtime["experiment_id"] = "v391-v386-safe-static-materializer"
    ...
    df = pd.read_csv(CSV)
    df["experiment_id"] = "v391-v386-safe-static-materializer"
    df["kernel_id"] = NEW_KERNEL_ID
    df["decision"] = audit["decision"]
    df.to_csv(CSV, index=False)

    report = REPORT.read_text(encoding="utf-8")
    report = report.replace("# v383 v380 Dry-run-tolerant Static Materializer", "# v391 v386 Safe Static Materializer")
    report = report.replace("v383-v380-dryrun-tolerant", "v391-v386-safe-static-materializer")
    report = report.replace("Can the v380 materializer be repaired so Kaggle Run-mode dry-run\n            train-row fallback no longer crashes while preserving real\n            sample-row guards and the clean v107 runtime policy?", "Can the safe-final v386 parameter set be materialized as a static CPU-only notebook while preserving the dry-run tolerant real sample-row guard?")
