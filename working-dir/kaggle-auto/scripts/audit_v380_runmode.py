#!/usr/bin/env python3
"""Audit v380 Run-mode schema/runtime output."""
...
OUT_DIR = ROOT / "birdclef-2026/outputs/v380-v379-static-distill-runmode"
STATUS_PATH = ROOT / "experiments/v380_runmode_status.md"
RUNTIME_PATH = ROOT / "artifacts/runtime_v380_runmode_status.json"
LINEAGE_PATH = ROOT / "artifacts/lineage_v380_runmode_status.json"
SAMPLE_PATH = ROOT / "birdclef-2026/data/sample_submission.csv"
DIAGNOSTICS = ["v380_static_distill_diagnostics.csv", "v380_static_distill_summary.csv", "submission_v107_anchor.csv"]
...
def log_flags(out_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "v380_printed": "v380 static distill patch complete" in text,
...
def write_status(status: str, fetched: bool, submission: dict[str, object], diagnostics: dict[str, dict[str, object]], flags: dict[str, object], runtime_sec: int | None, dec: str) -> None:
    ...
    lines += [
        ...
        "## Submit Gate",
        "",
        "No real competition submission is made by this audit. v380 may become a candidate for later guarded submission only if this audit reports READY and a separate competition-submit gate is explicitly authorized.",
    ]
    ...
    RUNTIME_PATH.write_text(
        json.dumps(
            {
                "experiment_id": "v380_runmode_status",
                "updated_utc": now_utc(),
                "kernel": KERNEL,
                "status": status,
                "fetched": fetched,
    ...
    LINEAGE_PATH.write_text(
        json.dumps(
            {
                "experiment_id": "v380_runmode_status",
                "source_notebook": "birdclef-2026/notebooks/v380-v379-static-distill",
