#!/usr/bin/env python3
"""Audit v391 Run-mode schema/runtime output."""
...
OUT_DIR = ROOT / "birdclef-2026/outputs/v391-v386-safe-static-distill-runmode"
STATUS_PATH = ROOT / "experiments/v391_runmode_status.md"
RUNTIME_PATH = ROOT / "artifacts/runtime_v391_runmode_status.json"
LINEAGE_PATH = ROOT / "artifacts/lineage_v391_runmode_status.json"
SAMPLE_PATH = ROOT / "birdclef-2026/data/sample_submission.csv"
DIAGNOSTICS = ["v391_static_distill_diagnostics.csv", "v391_static_distill_summary.csv", "submission_v107_anchor.csv"]
...
def log_flags(out_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "v391_printed": "v391 dry-run tolerant static distill patch complete" in text,
        "dryrun_row_mismatch_tolerated": "v391 dry-run row mismatch tolerated" in text,
...
def decision(status: str, submission: dict[str, object], diagnostics: dict[str, dict[str, object]], flags: dict[str, object], runtime_sec: int | None) -> str:
    ...
    if "STATUS_ERROR" in status_upper and not (submission.get("available") and flags.get("v391_printed")):
        return "WAIT-status-error"
    if any(term in status_upper for term in ['"STATUS": "CANCEL', '"STATUS": "ERROR', '"STATUS": "FAILED']):
        return "REJECT-v391-runmode-failed"
    local_complete = submission.get("available") and flags.get("v391_printed") and not flags.get("traceback")
    if "COMPLETE" not in status_upper and not local_complete:
        return "WAIT-running"
    if flags.get("traceback"):
        return "REJECT-v391-log-traceback"
    if flags.get("unknown_license_dataset_text"):
        return "REJECT-v391-unknown-license-runtime-text"
    missing_diag = [name for name, rep in diagnostics.items() if not rep.get("available")]
    if missing_diag:
        return "HOLD-v391-diagnostics-missing"
    if not schema_ok(submission):
        if submission.get("available") and submission.get("matches_sample_columns") and submission.get("contains_train_rows") and flags.get("dryrun_row_mismatch_tolerated"):
            if runtime_sec is not None and runtime_sec <= TARGET_RUNTIME_SEC:
                return "READY-v391-runmode-dryrun-runtime-proof"
            return "HOLD-v391-dryrun-runtime-unverified"
        return "REJECT-v391-submission-schema"
    if runtime_sec is not None and runtime_sec > HARD_RUNTIME_SEC:
        return "REJECT-v391-runtime-hardcap"
    if runtime_sec is not None and runtime_sec <= TARGET_RUNTIME_SEC:
        return "READY-v391-runmode-schema-runtime-proof"
    return "HOLD-v391-runtime-unverified"
...
def write_status(status: str, fetched: bool, submission: dict[str, object], diagnostics: dict[str, dict[str, object]], flags: dict[str, object], runtime_sec: int | None, dec: str) -> None:
    ...
    lines += [
        ...
        "## Submit Gate",
        "",
        "No real competition submission is made by this audit. v391 may become a candidate for later guarded submission only if this audit reports READY and a separate competition-submit gate is explicitly authorized.",
    ]
    ...
    RUNTIME_PATH.write_text(
        json.dumps(
            {
                "experiment_id": "v391_runmode_status",
                "updated_utc": now_utc(),
                "kernel": KERNEL,
                "status": status,
                "fetched": fetched,
    ...
    LINEAGE_PATH.write_text(
        json.dumps(
            {
                "experiment_id": "v391_runmode_status",
                "source_notebook": "birdclef-2026/notebooks/v391-v386-safe-static-distill",
