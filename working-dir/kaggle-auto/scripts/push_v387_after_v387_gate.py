#!/usr/bin/env python3
"""Gate and optionally push v387 for Kaggle Run-mode schema/runtime proof."""
...
V387_RUNTIME = ROOT / "artifacts/runtime_v387_v386_static_materializer_20260524.json"
STATUS = ROOT / "experiments/v387_after_v387_push_gate_status.md"
...
def decide() -> tuple[str, str, dict[str, object]]:
    runtime = read_json(V387_RUNTIME)
    ...
    evidence = {
        "v387_decision": runtime.get("decision", "MISSING"),
        "v387_block_count": audit.get("block_count", "MISSING"),
        "v387_macro_gain": metrics.get("macro_gain", "MISSING"),
        "v387_top5_anchor": metrics.get("anchor_top5_hit", "MISSING"),
        "v387_top5_local": metrics.get("local_top5_hit", "MISSING"),
    ...
    if runtime.get("decision") != "READY-v387-v386-static-materializer-audit-NO-PUSH-NO-SUBMIT":
        return "WAIT-v387-static-audit", "v387 static audit is not READY", evidence
    if audit.get("block_count") != 0:
        return "WAIT-v387-static-blockers", "v387 static audit has blockers", evidence
    if missing:
        return "WAIT-v387-files-missing", "v387 notebook files are missing", evidence
    if metadata.get("id") != KERNEL:
        return "WAIT-v387-metadata-id-mismatch", "metadata id does not match expected v387 kernel", evidence
    if not (evidence["metadata_cpu"] and evidence["metadata_no_tpu"] and evidence["metadata_no_internet"]):
        return "WAIT-v387-metadata-risk", "metadata is not CPU/no-TPU/no-internet", evidence
    if "jaejohn/perch-meta" in str(metadata.get("dataset_sources", [])) or "tuckerarrants/bc2026-distilled-sed-public" in str(metadata.get("dataset_sources", [])):
        return "WAIT-v387-unknown-source-risk", "metadata contains unknown-license sources", evidence
    try:
        ...
        return "WAIT-v387-metrics-missing", "local metrics are missing or invalid", evidence
    if macro_gain < 0.0015:
        return "WAIT-v387-promotion-gate", "local macro gain no longer meets promotion gate", evidence
    if top5_local + 1e-12 < top5_anchor:
        return "WAIT-v387-top5-regression", "local top5 regressed", evidence
    if pending > 0:
    ...
    return "READY-push-v387-runmode", "v387 may start Kaggle Run-mode schema/runtime proof only", evidence
...
def main() -> int:
    ...
    executed = False
    if args.execute:
        if decision != "READY-push-v387-runmode":
