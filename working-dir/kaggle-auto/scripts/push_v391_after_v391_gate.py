#!/usr/bin/env python3
"""Gate and optionally push v391 for Kaggle Run-mode schema/runtime proof."""
...
NOTEBOOK_DIR = ROOT / "birdclef-2026/notebooks/v391-v386-safe-static-distill"
V391_RUNTIME = ROOT / "artifacts/runtime_v391_v386_safe_static_materializer_20260524.json"
STATUS = ROOT / "experiments/v391_after_v391_push_gate_status.md"
...
def decide() -> tuple[str, str, dict[str, object]]:
    runtime = read_json(V391_RUNTIME)
    ...
    evidence = {
        "v391_decision": runtime.get("decision", "MISSING"),
        "v391_block_count": audit.get("block_count", "MISSING"),
        "v391_macro_gain": metrics.get("macro_gain", "MISSING"),
        "v391_top5_anchor": metrics.get("anchor_top5_hit", "MISSING"),
        "v391_top5_local": metrics.get("local_top5_hit", "MISSING"),
    ...
    if runtime.get("decision") != "READY-v391-v386-safe-static-materializer-audit-NO-PUSH-NO-SUBMIT":
        return "WAIT-v391-static-audit", "v391 static audit is not READY", evidence
    if audit.get("block_count") != 0:
        return "WAIT-v391-static-blockers", "v391 static audit has blockers", evidence
    if missing:
        return "WAIT-v391-files-missing", "v391 notebook files are missing", evidence
    if metadata.get("id") != KERNEL:
        return "WAIT-v391-metadata-id-mismatch", "metadata id does not match expected v391 kernel", evidence
    if not (evidence["metadata_cpu"] and evidence["metadata_no_tpu"] and evidence["metadata_no_internet"]):
        return "WAIT-v391-metadata-risk", "metadata is not CPU/no-TPU/no-internet", evidence
    if "jaejohn/perch-meta" in str(metadata.get("dataset_sources", [])) or "tuckerarrants/bc2026-distilled-sed-public" in str(metadata.get("dataset_sources", [])):
        return "WAIT-v391-unknown-source-risk", "metadata contains unknown-license sources", evidence
    try:
        ...
        return "WAIT-v391-metrics-missing", "local metrics are missing or invalid", evidence
    if macro_gain < 0.0015:
        return "WAIT-v391-promotion-gate", "local macro gain no longer meets promotion gate", evidence
    if top5_local + 1e-12 < top5_anchor:
        return "WAIT-v391-top5-regression", "local top5 regressed", evidence
    if pending > 0:
    ...
    return "READY-push-v391-runmode", "v391 may start Kaggle Run-mode schema/runtime proof only", evidence
...
def main() -> int:
    ...
    executed = False
    if args.execute:
        if decision != "READY-push-v391-runmode":
